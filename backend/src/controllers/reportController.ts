...
class ReportController {
  ...
  createBackup = async (req: Request, res: Response) => {
    try {
      ...
      const transactions = await TransactionModel.find({}).lean() as any[];
      const graphNodes = await GraphNodeModel.find({}).lean() as any[];
      const graphEdges = await GraphEdgeModel.find({}).lean() as any[];
  ...
  private restoreBackupData = async (backup: any) => {
    const { products, customers, transactions, graphNodes, graphEdges } = backup.data;
